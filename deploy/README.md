...
При обновлении до 0.5 также обновите unit из `crush-city-geo.service` и `proxy_read_timeout` до 90 секунд в конфиге этого домена. Выполните `systemctl daemon-reload`, проверьте `nginx -t` и перезагрузите Nginx. Скрипт активации перезапустит API с новыми ограничениями памяти. Перед заменой сохраните копии действующих конфигов.
...
Служба работает от `www-data`, слушает только localhost, имеет лимит памяти 1 ГБ (V8 heap 512 МБ, MemoryHigh 768 МБ). Кэш в `/var/cache/crush-city-geo` создаётся systemd, хранится между релизами и ограничен 100 файлами / 128 МБ. Геометрия хранится 7 дней, поиск — сутки. Данные одного ответа ограничены 128 МБ; таймаут источника 75 секунд, Nginx — 90 секунд. Одновременно передаётся одна большая карта, чтобы параллельные загрузки не исчерпали память службы. Одновременно выполняется один запрос геометрии, не чаще одного нового участка за 4 секунды; поиск — не чаще одного нового запроса за 1,2 секунды. На IP действует предел 30 запросов в минуту. Nginx обязательно перезаписывает `X-Real-IP`, не доверяя заголовку клиента.
