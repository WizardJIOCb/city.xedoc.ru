...
const AGENT = 'CrushCity/0.4 (+https://city.xedoc.ru; https://github.com/WizardJIOCb/city.xedoc.ru)';
const SIZES = [600, 1200, 1800], MAX_BYTES = 16 * 1024 * 1024;
export function parseArea(params) {
  ...
  if (!Number.isFinite(lat) || !Number.isFinite(lon) || Math.abs(lat) > 80 || Math.abs(lon) > 179.9 || !SIZES.includes(size)) throw new Error('Некорректные координаты или размер участка.');
...
export function areaQuery({ lat, lon, size }) {
  ...
  return `[out:json][timeout:25][maxsize:16777216];(way[building](${box});relation[building](${box});way[highway](${box});way[natural~"^(water|coastline|wood)$"](${box});relation[natural~"^(water|wood)$"](${box});way[waterway](${box});relation[waterway=riverbank](${box});way[landuse~"^(forest|grass|meadow|recreation_ground)$"](${box});way[leisure~"^(park|garden)$"](${box});node[natural=tree](${box}););out geom;`;
}

async function fetchJSON(url, init = {}, request = fetch) {
  const response = await request(url, { ...init, headers: { 'User-Agent': AGENT, ...init.headers }, signal: AbortSignal.timeout(35000) });
...
export function createGeoHandler({ cacheDir = process.env.GEO_CACHE_DIR || resolve('work/geo-cache'), request = fetch,
  ...
  const pending = new Map(), requests = new Map(); let nextSearch = 0, nextArea = 0, busyArea = false;
  ...
  return async function handle(req, res) {
    ...
    try {
      ...
      const result = await cached(`area:v1:${area.lat}:${area.lon}:${area.size}`, 7 * 86400000, async () => {
