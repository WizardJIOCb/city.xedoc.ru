...
test('map requests require finite bounded coordinates and one of three allowed sizes', () => {
  for (const query of ['', 'lat=&lon=1&size=600', 'lat=Infinity&lon=1&size=600', 'lat=81&lon=1&size=600', 'lat=0&lon=180&size=600', 'lat=0&lon=0&size=10000', 'lat=NaN&lon=0&size=600']) assert.throws(() => parseArea(new URLSearchParams(query)));
  ...
  const query=areaQuery({lat:0,lon:0,size:600}); assert.match(query, /timeout:25/); assert.match(query,/maxsize:16777216/); assert.match(query,/out geom/);
});
...
test('partial, oversized, failed and timed-out upstream replies never become playable cached maps',async t=>{
  for(const reply of [()=>json({remark:'runtime error',elements:[]}),()=>new Response('',{status:429}),()=>new Response('{}',{headers:{'content-length':String(17*1024*1024)}}),()=>{throw new DOMException('timeout','TimeoutError')}]){
...
test('streamed provider payload is bounded even without a Content-Length header',async t=>{
  const {get}=await setup(t,()=>new Response(new ReadableStream({start(controller){controller.enqueue(new Uint8Array(17*1024*1024));controller.close()}})));
