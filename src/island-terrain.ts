...
export function createIslandTerrain(radius: number, phase: number) {
  ...
  for (let i = 0; i < segments; i++) {
    ...
    triangle([0, top, 0], inner[next], inner[i], grass);
  ...
  material.onBeforeCompile = shader => {
    ...
    shader.vertexShader = shader.vertexShader
      .replace('#include <common>', '#include <common>\nvarying vec3 vIslandPosition;')
      .replace('#include <begin_vertex>', '#include <begin_vertex>\nvIslandPosition=position;');
    shader.fragmentShader = shader.fragmentShader
      .replace('#include <common>', '#include <common>\nvarying vec3 vIslandPosition; uniform float uRoadEnd; uniform vec3 uRoadColor;')
      .replace('#include <color_fragment>', `#include <color_fragment>
        ...
        float road=(1.-smoothstep(-edge,edge,distanceToRoad))*step(.69,vIslandPosition.y);
        diffuseColor.rgb=mix(diffuseColor.rgb,uRoadColor,road);
      `);
  };
  const terrain = new T.Mesh(geometry, material);
  terrain.name = 'island-terrain'; terrain.receiveShadow = true;
  return terrain;
}
