...
let fpsEnabled = true, shakeEnabled = true, flashEnabled = true, sunEnabled = true, raysEnabled = true, toastTimer = 0, fpsFrames = 0, fpsTime = 0, hudClock = 0, mapClock = 0, generating = false, lastNow = performance.now();
...
function homeCamera() { const e = city.extent; controls.maxDistance = Math.max(4400, e * 5); camera.far = Math.max(8500, e * 10); camera.updateProjectionMatrix(); (scene.fog as T.FogExp2).density = .00016 / Math.max(1, e / 900); camera.position.set(e * 1.48, e * 1.20, e * 1.78); controls.target.set(0, 8, 0); controls.update(); }
homeCamera();
...
function pointAt(x: number, y: number) { const r = canvas.getBoundingClientRect(); pointer.set((x - r.left) / r.width * 2 - 1, -(y - r.top) / r.height * 2 + 1); raycaster.setFromCamera(pointer, camera); aimedPlane = null; const air = raycaster.intersectObjects(city.planes.filter(p => p.userData.alive), true); if (air.length) { let parent = air[0].object; while (parent.parent && !city.planes.includes(parent as T.Group)) parent = parent.parent; aimedPlane = parent as T.Group; hitPoint.copy(air[0].point); return hitPoint; } if (city.realMap) { if (!raycaster.ray.intersectPlane(ground, hitPoint)) return null; const contact = city.collision.sweep(camera.position, hitPoint, 0); if (contact) hitPoint.lerpVectors(camera.position, hitPoint.clone(), contact.t); hitPoint.y = .8; return hitPoint; } const hits = raycaster.intersectObjects([city.facade.mesh]); if (hits.length) { hitPoint.copy(hits[0].point); hitPoint.y = .8; return hitPoint; } return raycaster.ray.intersectPlane(ground, hitPoint); }
...
function saveSettings() { try { localStorage.setItem('crushcity.settings', JSON.stringify({ quality, fpsEnabled, shakeEnabled, flashEnabled, sunEnabled, raysEnabled, sound: sound.enabled, soundPreferenceVersion: 1, blood: effects.bloodEnabled })); } catch { /* Storage is optional. */ } }
el<HTMLInputElement>('fps-enabled').onchange = e => { fpsEnabled = (e.target as HTMLInputElement).checked; el('fps-counter').hidden = !fpsEnabled; saveSettings(); };
...
try { const s = JSON.parse(localStorage.getItem('crushcity.settings') || '{}'); if (['low', 'medium', 'high'].includes(s.quality)) { el<HTMLSelectElement>('quality').value = s.quality; setQuality(s.quality, false); } if (s.fpsEnabled === false) { el<HTMLInputElement>('fps-enabled').checked = fpsEnabled = false; el('fps-counter').hidden = true; } if (s.sunEnabled === false) el<HTMLInputElement>('sun-enabled').checked = sunEnabled = false; if (s.raysEnabled === false) el<HTMLInputElement>('rays-enabled').checked = raysEnabled = false; if (s.shakeEnabled === false) el<HTMLInputElement>('shake-enabled').checked = shakeEnabled = false; if (s.flashEnabled === false) el<HTMLInputElement>('flash-enabled').checked = flashEnabled = false; if (s.blood === false) { effects.setBloodEnabled(false); el<HTMLInputElement>('blood-enabled').checked = false; } if (s.soundPreferenceVersion === 1 && s.sound === true) sound.enabled = true; } catch { /* Ignore corrupt settings. */ }
...
function frame(now: number) {
  ...
  if (!generating) {
    ...
    const near = Math.max(1, camera.position.distanceTo(controls.target) / 100); if (Math.abs(camera.near - near) > near * .1) { camera.near = near; camera.updateProjectionMatrix(); }
    night = T.MathUtils.lerp(night, nightTarget, Math.min(1, rawDt * 1.4)); city.night.value = night;
    sky.update(camera.position, night, sunEnabled); sun.target.position.copy(controls.target); sun.target.updateMatrixWorld(); sun.position.copy(sky.sunDirection).multiplyScalar(800).add(controls.target);
    ...
    ao.enabled = quality === 'high' && viewDistance < 4000;
    renderer.shadowMap.enabled = quality !== 'low' && viewDistance < 5000;
    sunRays.direction.copy(sky.sunDirection); sunRays.daylight = Math.pow(1 - night, 1.5); sunRays.enabled = raysEnabled && sunRays.daylight > .005 && viewDistance < 6000;
  ...
  if (hudClock > .3) { updateHUD(); hudClock = 0; } if (mapClock > (city.buildings.length > 10000 ? 2 : .5)) { drawMinimap(); mapClock = 0; }
}
