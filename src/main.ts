...
const controls = new OrbitControls(camera, canvas); controls.enableDamping = true; controls.dampingFactor = .07; controls.minDistance = 45; controls.maxDistance = 2200; controls.maxPolarAngle = Math.PI / 2 - .07; controls.minPolarAngle = .13; controls.panSpeed = .8; controls.rotateSpeed = .7; controls.zoomSpeed = .85;
...
const waterMat = new T.ShaderMaterial({ transparent: true, uniforms: { uTime: { value: 0 }, uNight: { value: 0 }, uCamera: { value: camera.position }, uFlood: { value: 0 } }, vertexShader: `uniform float uTime;varying vec3 vWorld;void main(){vec3 p=position;p.z+=sin(p.x*.021+uTime*.65)*.85+sin(p.y*.019+uTime*.5)*.65;vec4 w=modelMatrix*vec4(p,1.);vWorld=w.xyz;gl_Position=projectionMatrix*viewMatrix*w;}`, fragmentShader: `uniform float uTime;uniform float uNight;uniform vec3 uCamera;varying vec3 vWorld;void main(){vec2 p=vWorld.xz;float wave=sin(p.x*.021+uTime*.65)+sin(p.y*.019+uTime*.5);vec3 n=normalize(vec3(-cos(p.x*.021+uTime*.65)*.13,1.,-cos(p.y*.019+uTime*.5)*.12));vec3 view=normalize(uCamera-vWorld);float fresnel=pow(1.-max(0.,dot(n,view)),3.);vec3 color=mix(vec3(.012,.095,.12),vec3(.11,.27,.32),fresnel);float spec=pow(max(0.,dot(reflect(-normalize(vec3(-.48,.75,.55)),n),view)),150.);color+=vec3(1.,.80,.48)*spec*.9;float sparkle=pow(max(0.,sin(p.x*.39+sin(p.y*.17+uTime)))*max(0.,sin(p.y*.4)),18.);color+=sparkle*.13+wave*.011;color=mix(color,color*vec3(.28,.36,.58),uNight*.83);float fog=1.-exp(-length(uCamera-vWorld)*.00025);color=mix(color,mix(vec3(.26,.42,.48),vec3(.025,.05,.1),uNight),fog);gl_FragColor=vec4(color,.97);}` });
const water = new T.Mesh(new T.PlaneGeometry(8000, 8000, 150, 150), waterMat); water.rotation.x = -Math.PI / 2; water.position.y = -2; scene.add(water);
...
function setSelected(id: DisasterId) { selected = id; selectCard(id); const d = DISASTERS.find(d => d.id === id)!; const color = new T.Color(d.color); (targetRing.material as T.MeshBasicMaterial).color.copy(color); (innerRing.material as T.MeshBasicMaterial).color.copy(color); sound.select(); }
function pointAt(x: number, y: number) { const r = canvas.getBoundingClientRect(); pointer.set((x - r.left) / r.width * 2 - 1, -(y - r.top) / r.height * 2 + 1); raycaster.setFromCamera(pointer, camera); const hits = raycaster.intersectObject(city.facade.mesh); if (hits.length) { hitPoint.copy(hits[0].point); hitPoint.y = .8; return hitPoint; } return raycaster.ray.intersectPlane(ground, hitPoint); }
const down = { x: 0, y: 0, time: 0 }; let touches = 0, gesture = false;
canvas.addEventListener('pointermove', e => { if (pointAt(e.clientX, e.clientY)) { target.position.set(hitPoint.x, 1.5, hitPoint.z); targetCross.position.set(hitPoint.x, 1.6, hitPoint.z); const def = DISASTERS.find(d => d.id === selected)!; target.scale.setScalar(def.radius * Math.sqrt(effects.power)); target.visible = !cinematic && Math.abs(hitPoint.x) < city.extent + 180 && Math.abs(hitPoint.z) < city.extent + 180; targetCross.visible = target.visible; } });
...
canvas.addEventListener('pointerup', e => {
  ...
  if (pointAt(e.clientX, e.clientY) && Math.abs(hitPoint.x) < city.extent + 180 && Math.abs(hitPoint.z) < city.extent + 180) effects.trigger(selected, hitPoint.x, hitPoint.z);
});
...
for (const b of document.querySelectorAll<HTMLElement>('[data-category]')) b.onclick = () => { for (const tab of document.querySelectorAll('[data-category]')) tab.classList.toggle('active', tab === b); for (const card of document.querySelectorAll<HTMLElement>('[data-disaster]')) card.hidden = b.dataset.category !== 'all' && card.dataset.type !== b.dataset.category; };
el<HTMLInputElement>('power').oninput = e => { effects.power = Number((e.target as HTMLInputElement).value); el('power-value').textContent = effects.power.toFixed(2).replace(/0$/, '') + '×'; };
...
async function regenerate(seed: string, size: number, style: string) {
  ...
  try {
    effects.reset(); city.dispose(); city = new City(scene, seed, size, style === 'islands' ? 'islands' : 'bay'); effects.attachCity(city); simTime = 0; preset = style; homeCamera(); setDay(style === 'night' ? 2 : 0); pause(false); updateHUD(); drawMinimap();
...
function saveSettings() { try { localStorage.setItem('crushcity.settings', JSON.stringify({ quality, shakeEnabled, flashEnabled, sound: sound.enabled, blood: effects.bloodEnabled })); } catch { /* Storage is optional. */ } }
...
try { const s = JSON.parse(localStorage.getItem('crushcity.settings') || '{}'); if (['low', 'medium', 'high'].includes(s.quality)) { el<HTMLSelectElement>('quality').value = s.quality; setQuality(s.quality, false); } if (s.shakeEnabled === false) el<HTMLInputElement>('shake-enabled').checked = shakeEnabled = false; if (s.flashEnabled === false) el<HTMLInputElement>('flash-enabled').checked = flashEnabled = false; if (s.blood === false) { effects.setBloodEnabled(false); el<HTMLInputElement>('blood-enabled').checked = false; } if (s.sound === false) { sound.enabled = false; el('sound').innerHTML = icon('mute'); } } catch { /* Ignore corrupt settings. */ }
function resize() { camera.aspect = innerWidth / innerHeight; if (!cinematic) camera.setViewOffset(innerWidth, innerHeight, 0, Math.min(95, innerHeight * .12), innerWidth, innerHeight); else camera.clearViewOffset(); camera.updateProjectionMatrix(); renderer.setSize(innerWidth, innerHeight); composer.setPixelRatio(renderer.getPixelRatio()); composer.setSize(innerWidth, innerHeight); effects.fire.material.uniforms.uScale.value = innerHeight; effects.smoke.material.uniforms.uScale.value = innerHeight; effects.blood.material.uniforms.uScale.value = innerHeight; }
...
function updateHUD() {
  ...
  const n = effects.events.filter(e => !['ring', 'bolt'].includes(e.type)).length;
...
mini.onclick = e => { const r = mini.getBoundingClientRect(), x = (e.clientX - r.left) / r.width * mini.width, y = (e.clientY - r.top) / r.height * mini.height, scale = Math.min(mini.width, mini.height) / (city.extent * 2.4); const tx = (x - mini.width / 2) / scale, tz = (y - mini.height / 2) / scale; camera.position.x += tx - controls.target.x; camera.position.z += tz - controls.target.z; controls.target.x = tx; controls.target.z = tz; };
...
function frame(now: number) {
  ...
  if (!generating) {
    simTime += dt; city.update(dt, simTime); effects.update(dt, simTime);
    ...
    camera.position.add(move); controls.target.add(move); controls.target.x = T.MathUtils.clamp(controls.target.x, -city.extent * 1.5, city.extent * 1.5); controls.target.z = T.MathUtils.clamp(controls.target.z, -city.extent * 1.5, city.extent * 1.5); controls.update();
    night = T.MathUtils.lerp(night, nightTarget, Math.min(1, rawDt * 1.4)); city.night.value = night; waterMat.uniforms.uTime.value = simTime; waterMat.uniforms.uNight.value = night; skyMat.uniforms.uNight.value = night; water.position.y = -2 + effects.flood;
