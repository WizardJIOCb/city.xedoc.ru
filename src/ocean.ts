...
export class Ocean {
  ...
  constructor(scene: T.Scene, camera: T.Camera) {
    this.material = new T.ShaderMaterial({
      uniforms: { uTime: { value: 0 }, uNight: { value: 0 }, uFogScale: { value: 1 }, uCamera: { value: camera.position }, uSunDirection: { value: new T.Vector3(-.48, .75, .55).normalize() }, uIslands: { value: Array.from({ length: 6 }, () => new T.Vector4(0, 0, 0, 0)) }, uShips: { value: Array.from({ length: 8 }, () => new T.Vector4(0, 0, 0, 0)) } },
      ...
      fragmentShader: `uniform float uTime;uniform float uNight;uniform float uFogScale;uniform vec3 uCamera,uSunDirection;uniform vec4 uIslands[6];uniform vec4 uShips[8];varying vec3 vWorld;
        ...
        void main(){vec2 p=vWorld.xz;float d=length(uCamera-vWorld);float fade=1.-smoothstep(900.,3900.,d);
          ...
          float fog=1.-exp(-d*.00022/uFogScale);color=mix(color,mix(vec3(.30,.46,.51),vec3(.035,.065,.12),uNight),fog);gl_FragColor=vec4(color,1.);
  ...
  update(time: number, night: number, city: City) {
    ...
    const scale = Math.max(1, city.extent / 700); this.mesh.scale.set(scale, 1, scale); this.material.uniforms.uFogScale.value = Math.max(1, city.extent / 900);
    this.material.uniforms.uIslands.value.forEach((v: T.Vector4, i: number) => {
      const island = city.islands[i];
      if (island) v.set(island.x, island.z, island.radius, island.phase);
      else v.set(0, 0, 0, 0);
    });
