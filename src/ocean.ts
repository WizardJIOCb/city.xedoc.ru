...
export class Ocean {
  ...
  constructor(scene: T.Scene, camera: T.Camera) {
    this.material = new T.ShaderMaterial({
      uniforms: { uTime: { value: 0 }, uNight: { value: 0 }, uCamera: { value: camera.position }, uIslands: { value: Array.from({ length: 6 }, () => new T.Vector4(0, 0, 0, 0)) }, uShips: { value: Array.from({ length: 8 }, () => new T.Vector4(0, 0, 0, 0)) } },
      ...
      fragmentShader: `uniform float uTime;uniform float uNight;uniform vec3 uCamera;uniform vec4 uIslands[6];uniform vec4 uShips[8];varying vec3 vWorld;
        ...
        void main(){vec2 p=vWorld.xz;float d=length(uCamera-vWorld);float fade=1.-smoothstep(900.,3900.,d);
          ...
          vec3 light=normalize(vec3(-.48,.75,.55));float sun=pow(max(0.,dot(reflect(-light,n),v)),95.);color+=vec3(1.,.83,.55)*sun*.75;
    ...
    this.mesh = new T.Mesh(geometry, this.material); this.mesh.position.y = -2; scene.add(this.mesh);
  }
